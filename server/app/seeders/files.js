...
async function seedDB() {
    ...
    try {
      await client.startSession();
        ...
        const buckets = client.db("db").collection("buckets");

        const randomBucket = await buckets.findOne()
        // make a bunch of time series data
        let timeSeriesData = [];

        for (let i = 0; i < 5000; i++) {
            const fileName = faker.system.fileName();
            const fileType = faker.system.fileType()
            let newFile = {
                bucketKey: randomBucket._id,
                fileKey: generateAPIKey(),
               fileName: fileName,
               fileType: fileType
            };
